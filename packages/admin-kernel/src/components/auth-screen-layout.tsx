...
export function AuthScreenLayout({
      ...
      <div className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#ffffff_100%)] px-4 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-[560px] items-center justify-center">
          <div className="w-full space-y-5">
            ...
            <div className="rounded-[10px] border border-slate-200 bg-white p-6 shadow-[0_16px_48px_rgba(15,23,42,0.08)] sm:p-7">
              {children}
            </div>

            {footer ? (
              <div className="text-center text-sm leading-6 text-slate-500">{footer}</div>
            ) : null}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[color:var(--color-canvas)] p-3 sm:p-4 lg:p-6">
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-[1540px] overflow-hidden rounded-[12px] border border-[color:var(--color-border)] bg-white shadow-[0_20px_80px_rgba(15,23,42,0.08)] lg:grid-cols-[1.08fr_0.92fr]">
          ...
          <div className="relative flex h-full flex-col justify-between gap-8">
            <div className={hasHeroMetrics ? "space-y-8" : "space-y-5"}>
              <div className="flex items-center justify-between gap-4">
                ...
                <Badge className="border-white/10 bg-white/5 text-slate-100 shadow-none">
                  {eyebrow}
                </Badge>
              </div>
              ...
              <div className="max-w-2xl space-y-4">
                ...
                <p className="max-w-xl text-sm leading-7 text-slate-300 sm:text-base">{heroBody}</p>
              </div>
              ...
              {hasHeroMetrics ? (
                <div className="grid gap-3 sm:grid-cols-3">
                  {heroMetrics?.map((metric) => (
                    <div
                      key={metric.label}
                      className="rounded-sm border border-white/10 bg-white/5 px-4 py-4 backdrop-blur-sm"
                    >
            ...
            {hasHeroHighlights ? (
              <div className="grid gap-3 xl:grid-cols-2">
                {heroHighlights?.map((highlight) => (
                  <div
                    key={highlight.title}
                    className="rounded-sm border border-white/10 bg-white/5 p-4 backdrop-blur-sm"
                  >
                    <div className="flex items-start gap-3">
                      <span className="inline-flex h-10 w-10 items-center justify-center rounded-sm bg-white/10 text-sky-100">
