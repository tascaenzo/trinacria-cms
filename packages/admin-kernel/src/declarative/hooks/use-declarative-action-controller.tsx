...
type DraftFieldValue = string | boolean | string[];

const defaultTranslate: TranslateFn = (key, fallback) => fallback ?? key;

interface DeclarativeActionOption {
  value: string;
  label: string;
  description?: string;
  meta?: string;
...
interface DeclarativeActionOptionSource {
...
interface DeclarativeActionFormField {
  key: string;
  label: string;
...
interface DeclarativeActionOptionState {
...
function DeclarativeActionFormField({
  field,
  onChange,
  optionState,
  value
}: {
  field: DeclarativeActionFormField;
...
): readonly DeclarativeActionFormField[] {
...
function defaultDraftFieldValue(field: DeclarativeActionFormField): DraftFieldValue {
  if (field.kind === "boolean") {
    return false;
  }
  if (field.kind === "stringArray") {
...
function createActionBodyFromFields(
  action: DeclarativeAction,
...
function parseDraftFieldValue(field: DeclarativeActionFormField, value: DraftFieldValue): unknown {
  if (field.kind === "boolean") {
    return Boolean(value);
  }
