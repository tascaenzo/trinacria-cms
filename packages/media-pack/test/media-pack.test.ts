...
test("media-pack manifest declares its foundation contributions", () => {
  ...
  assert.deepEqual(manifest.admin?.widgets ?? [], []);
  assert.deepEqual(
    manifest.admin?.routes?.map(({ componentRef, label, path }) => ({ componentRef, label, path })),
    [{ componentRef: "media-pack:file-manager", label: "Media", path: "/media/assets" }]
  );
  assert.deepEqual(
    manifest.admin?.navigation?.map(({ label, path }) => ({ label, path })),
    [{ label: "Media", path: "/media/assets" }]
  );
  assert.deepEqual(
