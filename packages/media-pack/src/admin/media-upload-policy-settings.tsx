...
const DEFAULT_MIME_TYPES = ["image/jpeg", "image/png", "image/webp", "application/pdf", "text/plain", "text/csv"];
...
export function MediaUploadPolicySettings({ cms, t }: MediaUploadPolicySettingsContext) {
  ...
  async function readMimeTypes(): Promise<readonly string[]> {
    try {
      const response = await cms.settings.getSettingValueByKey({ path: { key: SETTINGS.allowedMimeTypes } });
  ...
  function addCustomMimeType() {
    ...
    setMimeTypes((current) => (current.includes(nextMimeType) ? current : [...current, nextMimeType]));
    setCustomMimeType("");
    setError(null);
    setMessage(null);
  }
    ...
    <section className="flex h-full min-h-0 flex-col">
        ...
        <div className="mx-auto grid max-w-4xl gap-6">
          ...
          <section className="grid gap-4 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Input
                ...
                onChange={(event) => { setMaxFileMegabytes(event.currentTarget.value); setMessage(null); }}
              />
              <Input
                ...
                readOnly={isLoading || isSaving}
                onChange={(event) => { setMaxImageMegapixels(event.currentTarget.value); setMessage(null); }}
              />
            </div>
          </section>
          ...
          <section className="grid gap-5 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h4 className="text-sm font-semibold text-[color:var(--color-ink)]">Tipi di file consentiti</h4>
                <p className="mt-1 text-sm text-[color:var(--color-ink-muted)]">
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {MIME_TYPE_GROUPS.map((group) => (
                <section key={group.label} className="grid content-start gap-2">
                  <h5 className="text-xs font-semibold uppercase tracking-[0.12em] text-[color:var(--color-ink-subtle)]">{group.label}</h5>
            ...
            <div className="grid gap-3 border-t border-[color:var(--color-border)] pt-4">
              <div>
                <h5 className="text-sm font-semibold text-[color:var(--color-ink)]">Formato personalizzato</h5>
                <p className="mt-1 text-sm text-[color:var(--color-ink-muted)]">Per formati non presenti nell&apos;elenco, ad esempio application/zip.</p>
              </div>
              <div className="flex flex-col gap-2 sm:flex-row">
                <Input
                  ...
                  onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustomMimeType(); } }}
                />
                <Button type="button" variant="secondary" disabled={isLoading || isSaving} onClick={addCustomMimeType}>Aggiungi</Button>
              </div>
              {customMimeTypes.length ? (
                <div className="flex flex-wrap gap-2">
                  {customMimeTypes.map((mimeType) => (
                    <button key={mimeType} type="button" disabled={isSaving} onClick={() => toggleMimeType(mimeType)} className="rounded-full border border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-3 py-1 text-xs text-[color:var(--color-ink-muted)] hover:bg-[color:var(--color-interactive-hover)]" aria-label={`Rimuovi ${mimeType}`}>
      ...
      <footer className="shrink-0 border-t border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-4 pb-2 pt-3 sm:px-5">
        <div className="mx-auto flex max-w-4xl justify-end gap-2">
          <Button type="button" variant="secondary" disabled={isLoading || isSaving} onClick={() => void loadPolicy()}>{t("common.actions.refresh", "Aggiorna")}</Button>
          <Button type="button" disabled={isLoading || isSaving} onClick={() => void savePolicy()}>{isSaving ? t("common.actions.saving", "Salvataggio...") : t("common.actions.save", "Salva")}</Button>
        </div>
      </footer>
    </section>
  );
...
function readPositiveNumber(value: string, label: string): number {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) throw new Error(`${label} deve essere maggiore di zero.`);
  return parsed;
}
...
function toDisplayError(error: unknown): string {
  return error instanceof Error ? error.message : "Impossibile aggiornare le regole di caricamento.";
}
