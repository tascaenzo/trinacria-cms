...
`blacklisted_tokens` collection:

```
Collection: blacklisted_tokens
Document:   { sub, iat, kind ("access"|"refresh"), expiresAt, createdAt }
Indexes:    unique on id, compound unique on (sub, iat), index on expiresAt
```
...
Failed login attempts are tracked in the `login_attempts` collection:

```
Collection: login_attempts
Document:   { email, count, lockoutUntil?, createdAt, updatedAt }
Indexes:    unique on id, unique on email
```
...
3. The `login_attempts` record is a simple counter with upsert semantics. The
...
- If your plugin provides its own auth mechanism, you should enforce similar
  protections.
- The `blacklisted_tokens` and `login_attempts` collections are managed
