...
| Admin UI manifest                | `AdminExtensionManifest`      | `manifest admin`                        | `admin manifest`                     | Serializable plugin-facing contract for backoffice pages, menus and widgets. |
| Safe admin UI manifest           | `SafeAdminExtensionManifest`  | `manifest admin safe`                   | `safe admin manifest`                | Manifest after sanitizer; accepted by the pure normalizer.                   |
| Backoffice renderable surface    | `RenderableAdminContribution` | `contribution renderizzabile`           | `renderable admin contribution`      | Internal mounted form produced from safe manifests or local React surfaces.  |
| Declarative admin renderer       | `Declarative Renderer`        | `renderer dichiarativo`                 | `declarative renderer`               | Standard UI generated from manifest metadata and schemas.                    |
| Local React admin escape hatch   | `contributions`               | `contributions`                         | `contributions`                      | Non-serializable local React surfaces; not the preferred plugin API path.    |
...
6. Use `AdminExtensionManifest` for plugin-exposed admin UI metadata.
7. Use `contributions` only for local React escape hatches, not for runtime plugin APIs.
